...
public interface TradeRepo extends JpaRepository<TradeEntity, Long> {
	...
	@Query(value = "select a from Trade a where a.userId = ?0")
	List<TradeEntity> findAllByUserId(Long userId);

	@Query(value = "select a from Trade a where a.symbol = ?0 AND a.type = ?1 AND (a.timestamp between ?2 and ?3)")
	List<TradeEntity> findTradeBySymbolAndType(String stockSymbol, String tradeType, Timestamp startDate,
			Timestamp endDate);

	@Query(value = "select a.symbol, a.price from Trade a where a.symbol = ?0 AND (a.timestamp between ?1 and ?2) order by a.symbol, a.price")
